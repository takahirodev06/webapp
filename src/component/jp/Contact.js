...
function Contact_Jp(props) {

    useEffect(() => {
        document.title = 'お問い合わせ | TAKAHIRO YAMADA WEB CREATION';
        window.scroll({
            top: 0,
            // behavior: "smooth",
        });
    ...
    return (
        <div className="sub_container contact_page jp">
            <h2>Contact <span> お問い合わせ</span></h2>
            <section className="contact_address">
                <p><span> メールでのお問い合わせはこちらから</span></p>
                <h3>
                    ...
                    {copyTocb ? <span onClick={copytocilpbord}>コピーしました</span> :
                        <span onClick={copytocilpbord}>メールアドレスをコピー</span>}
                </h3>
            </section>
                ...
                <form onSubmit={handleSubmit(onSubmit)} method="post" ref={form} id={"form_area"}>
                    <div className={"form_sending"}>
                        ...
                        <div className={"form_field_item"}>
                            ...
                            {errors.company && <p className="form_alert"><span> * 必須項目です。</span></p>}
                        </div>
                        <div className={"form_field_item"}>
                            <input type="text" placeholder="担当者氏名"
                            ...
                            {errors.name && <p className="form_alert"><span> * 必須項目です。</span></p>}
                        </div>
                        <div className={"form_field_item"}>
                            ...
                            {errors.email && <p className="form_alert"><span> * 必須項目です。</span></p>}
                            {!errors.email && emailcheck &&
                                <p className="form_alert"><span>* メールアドレスの形式が正しくありません。</span></p>}
                        </div>
                        <div className={"form_field_item"}>
                            <textarea placeholder="本文" className={"width100"}
                            ...
                            {errors.message && <p className="form_alert"><span> * 必須項目です。</span></p>}
                        </div>
                        <div className={"form_switch"}>
                            <button type="submit" value="" disabled={!submit}>送信</button>
                            <img src={arrowyellow} alt="" className={"arrowyellow"}/>

                        </div>
                    </div>
                    <div className={"form_is_send"}>
                    <p>お問い合わせありがとうございました。<br/>メールは送信されました。</p>
                        <div className={"form_switch"}>
                            <Link to="/jp">ホームへ</Link>
                        </div>
                    </div>
                </form>
